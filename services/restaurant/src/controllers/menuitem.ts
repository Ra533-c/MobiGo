...
export const addMenuItem = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  //here we are using multer to upload file
  //frontend -> file -> restaurant ser(backend) -> utils ser(mutler(buffer)->datauri(base64)->upload to cloud)
  const file = req.file;
