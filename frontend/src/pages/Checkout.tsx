...
import { restaurantService, utilsService } from "../main.js";
  ...
  const payWithStripe = async () => {
    try {
      ...
      try {
        ...
        console.log(`success_url data.url -> ${data.url}`);
    ...
    <div className="mx-auto max-w-4xl px-4  py-6 space-y-6">
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#2d7ff9] py-3 text-sm font-semibold text-white hover:bg-blue-400 disabled:opacity-50"
        >
          {loadingRazorpay ? <BiLoader size={18} className="animate-spin" /> : <BiCreditCard size={18} />}Pay With Razorpay
        </button>
        ...
        >
          {loadingStripe ? <BiLoader size={18} className="animate-spin" /> : <BiCreditCard size={18} />}Pay With Stripe
        </button>
      </div>
    </div>
  );
