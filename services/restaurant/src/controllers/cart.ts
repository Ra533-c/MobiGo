...
export const addToCart = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (cartFromDifferentRestaurant) {
    return res.status(400).json({
      message:
        "You can order from only one restaurant at a time. Please clear you cart first to add items from this restaurant.",
    });
  }
...
export const increamentCartItem = TryCatch(
...
export const decreamentCartItem = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const userId = req.user?._id;
